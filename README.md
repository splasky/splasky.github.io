...
GitHub-hosted images in the configured content repository are copied into the site. Vercel-hosted images are downloaded during the build; an unavailable image fails publication. Other external images remain external. Markdown supports GFM, math, syntax highlighting, tables and sanitized HTML. Blog and Thought Markdown supports standalone YouTube, public Google Drive, and IPFS video URLs. The reader emits a sanitized load card; no third-party player request is made until the reader clicks `載入影片`. Inline links, About content, unsupported URLs, and raw iframe/video HTML remain ordinary sanitized content. Google Drive files must be shared for viewers with the link, and IPFS videos should use a browser-supported MP4/H.264 or WebM codec. Unsafe protocols, scripts, iframes, path traversal, invalid metadata, duplicate IDs, broken local links and unintended Vercel dependencies fail validation. Project-local `.vscode/` links in existing notes are displayed as code text.
